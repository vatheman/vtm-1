...
- **지침**: 프로젝트 규칙과 AI 가이드라인 원본은 단 1곳(SSoT)에만 관리합니다.
- **실행**: 개별 AI 도구별 지시 파일(`GEMINI.md`, `AGENTS.md`, `.agents/rules/` 등)은 단일 원본을 공유/참조하도록 구성합니다.
- **목적**: 지침 파편화 및 충돌 방지.

### 2. 코드 변경 시 문서 동기화 필수 원칙 (`docs/` 업데이트)
- **지침**: 로직 및 구조 변경, 버그 수정 시 코드 수정만으로 완료하지 않고 관련 문서(`docs/`) 및 변경 이력을 동시 업데이트합니다.
- **실행**: 리팩토링 노트, 이슈 트러블슈팅 이력 및 차이 로그 작성.
- **목적**: 지식 파편화 방지 및 최신 실행 가능 문서 유지.

### 3. 복잡한 작업 시 Subagent(에이전트 역할 분담) 활용
- **지침**: 대규모/고난이도 작업 시 세부 역할(설계, 구현, 검증, 문서화)을 분담하여 처리합니다.
- **실행**: `invoke_subagent` 도구 또는 전용 스킬을 호출하여 유기적 역할 수행.
- **목적**: AI 맥락 손실(Context Overflow) 방지 및 완성도 극대화.
